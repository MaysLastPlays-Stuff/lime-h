...
/* SDL_ScanUnsignedLongLongInternal is defined below, only once, to avoid redefinition errors. */

/* Always declare the prototype for SDL_ScanUnsignedLongLongInternal so all static functions can use it. */
...
/* Always define SDL_ScanUnsignedLongLongInternal unconditionally, so it is only defined once. */
static size_t SDL_ScanUnsignedLongLongInternal(const char *text, int count, int radix, unsigned long long *valuep, SDL_bool *negativep)
{
